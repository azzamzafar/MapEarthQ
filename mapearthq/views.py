...
def index(request):
    ...
    for listing in listings:
        ...
        if listing.Mag>=6:
            ...
            folium.CircleMarker([listing.lat,listing.lon],radius=30,fill=True,color='red',opacity=0.6,popup=popupstr).add_to(m)
        else:
            folium.CircleMarker([listing.lat,listing.lon],radius=10,fill=True,color='blue',opacity=0.6).add_to(m)    
            folium.CircleMarker([listing.lat,listing.lon],radius=20,fill=True,color='blue',opacity=0.6,popup=popupstr).add_to(m)
